...
export const ELEMENT_MAPPINGS: ElementMapping[] = [
  // Layout containers
  {
    ...
    decisionGuide:
      'Use Stack for vertical/horizontal stacking, Grid for multi-column layouts, Container for max-width constraints, Card for card styling',
  ...
  },
  {
    ...
    decisionGuide:
      'Use DashboardSidebar for main navigation, Tabs for tab navigation, Breadcrumbs for breadcrumb nav',
  ...
  // Form elements
  {
    ...
    decisionGuide:
      'Use Textfield for text input, Checkbox for checkboxes, Radio for radio buttons, Search for search inputs',
    importFrom: '@digdir/designsystemet-react',
  ...
  },
  {
    ...
    decisionGuide:
      'Use NativeSelect for simple selects, Combobox for searchable selects, Dropdown for custom dropdowns',
    importFrom: '@digdir/designsystemet-react',
...
export function generateErrorMessage(element: string): string {
  ...
  const alts =
    mapping.alternatives.length > 0 ? ` (alternatives: ${mapping.alternatives.join(', ')})` : '';

  return (
    `Raw <${element}> is forbidden. Use ${mapping.preferred}${alts} instead. ` +
