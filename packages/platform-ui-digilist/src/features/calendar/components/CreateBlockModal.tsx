 ...
 * Presentational component - all data and actions passed via props.
 *
...
export interface CreateBlockModalProps {
  ...
  isOpen: boolean;
  /** Available rental objects for selection */
  rentalObjects: RentalObject[];
  /** Whether rental objects are loading */
  isLoadingRentalObjects?: boolean;
  /** User permissions */
  ...
  /** Whether create action is pending */
  isCreating?: boolean;
  /** Initial listing ID */
...
export function CreateBlockModal({
  ...
  onFormChange,
  onCreate,
}: CreateBlockModalProps): React.ReactElement {
  const t = useT();
  ...
  // Notify parent of form changes for conflict checking
  useEffect(() => {
    onFormChange?.(formData, showRecurrence, recurrence);
  }, [formData, showRecurrence, recurrence, onFormChange]);
    ...
    <Dialog open={isOpen} onClose={onClose}>
      <Dialog.Block>
        <Heading level={2} data-size="md">
          {t('backoffice.calendar.createBlock')}
        </Heading>
        ...
        <Stack spacing="4">
          ...
          <div>
            ...
            >
              <option value="">{t('backoffice.calendar.selectListing')}</option>
          ...
          <div>
            ...
            >
              {t('backoffice.calendar.blockType')} *
            </label>
          ...
          <div>
            ...
            <input
              ...
              placeholder={t('backoffice.calendar.titlePlaceholder')}
              style={{
                width: '100%',
                padding: 'var(--ds-spacing-2) var(--ds-spacing-3)',
                borderRadius: 'var(--ds-border-radius-md)',
          ...
          >
            <div>
              ...
              >
                {t('backoffice.calendar.fromDate')} *
              </label>
              <input
                id="start-date"
                type="date"
          ...
          {!formData.allDay && (
            ...
            >
              <div>
                ...
                >
                  {t('backoffice.calendar.fromTime')} *
                </label>
                <input
                  id="start-time"
                  type="time"
            ...
            <>
              <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--ds-spacing-2)' }}>
                <Checkbox
                  ...
                  aria-label={t('backoffice.calendar.repeatBlock')}
                />
                <span style={{ fontSize: 'var(--ds-font-size-sm)' }}>
                  {t('backoffice.calendar.repeatBlock')}
                </span>
              </div>
          ...
          <div>
            ...
            >
              {t('backoffice.calendar.notes')}
            </label>
            <textarea
              id="block-notes"
              value={formData.notes}
              onChange={(e) => updateField('notes', e.target.value)}
              placeholder={t('backoffice.calendar.notesPlaceholder')}
              rows={3}
              style={{
                width: '100%',
                padding: 'var(--ds-spacing-2) var(--ds-spacing-3)',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--ds-spacing-2)' }}>
            <Checkbox
              ...
              aria-label={t('backoffice.calendar.notifyAffectedUsers')}
            />
            <span style={{ fontSize: 'var(--ds-font-size-sm)' }}>
              {t('backoffice.calendar.notifyAffectedUsers')}
            </span>
          </div>
          ...
          {isCheckingConflicts && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--ds-spacing-2)' }}>
              <Spinner data-size="sm" aria-label={t('backoffice.calendar.checkingConflicts')} />
              <Paragraph
                data-size="sm"
                style={{ margin: 0, color: 'var(--ds-color-neutral-text-subtle)' }}
              >
                {t('backoffice.calendar.checkingConflicts')}
              </Paragraph>
            </div>
          )}
          ...
          {conflicts?.hasConflicts && (
            <Alert data-color="warning">
              ...
              <ul style={{ margin: 0, paddingLeft: 'var(--ds-spacing-4)' }}>
                {conflicts.conflicts.slice(0, 5).map((conflict, idx) => (
                  <li key={idx} style={{ fontSize: 'var(--ds-font-size-sm)' }}>
                    {conflict.title} (
                    {new Date(conflict.startTime).toLocaleTimeString('nb-NO', {
                      hour: '2-digit',
                      minute: '2-digit',
                    })}{' '}
                    -{' '}
                    {new Date(conflict.endTime).toLocaleTimeString('nb-NO', {
                      hour: '2-digit',
                      minute: '2-digit',
                    })}
                    )
      ...
      <Dialog.Block>
        <Stack direction="horizontal" spacing="3" style={{ justifyContent: 'flex-end' }}>
          ...
          >
            {isCreating ? t('backoffice.calendar.creating') : t('action.create')}
          </Button>
        </Stack>
      </Dialog.Block>
    </Dialog>
