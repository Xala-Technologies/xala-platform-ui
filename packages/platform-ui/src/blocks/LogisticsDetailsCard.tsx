...
export function LogisticsDetailsCard({
        ...
        return (
            <div style={fieldStyle}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 'var(--ds-spacing-2)' }}>
                    {icon && <span style={{ color: 'var(--ds-color-accent-base-default)', flexShrink: 0 }}>{icon}</span>}
    ...
    return (
        <Card className={cn('logistics-details-card', className)} style={{ padding: 'var(--ds-spacing-4)' }}>
            <div style={containerStyle}>
                ...
                {hasLogistics && (
                    <div style={sectionStyle}>
                        ...
                        {logistics?.pickupLocation && renderField(
                            labels.pickupLocation,
                            <>
                                <strong>{logistics.pickupLocation}</strong>
                                {logistics.pickupAddress && <><br />{logistics.pickupAddress}</>}
                            </>,
                            <MapPinIcon size={16} />
                        )}

                        {logistics?.pickupHours && renderField(
                            labels.pickupHours,
                            logistics.pickupHours,
                            <ClockIcon size={16} />
                        )}

                        {logistics?.deliveryAvailable !== undefined && renderField(
                            labels.delivery,
                            logistics.deliveryAvailable ? labels.deliveryAvailable : labels.deliveryNotAvailable
                        )}

                        {logistics?.transportInfo && renderField(
                            labels.transport,
                            logistics.transportInfo
                        )}
                    </div>
                ...
                {hasDetails && (
                    <div style={sectionStyle}>
                        ...
                        {details?.returnDeadline && renderField(
                            labels.returnDeadline,
                            details.returnDeadline
                        )}

                        {details?.damagePolicy && renderField(
                            labels.damagePolicy,
                            details.damagePolicy
                        )}
                    </div>
