...
export type RegistrationStatus = 'upcoming' | 'open' | 'closing_soon' | 'sold_out' | 'closed' | 'waitlist';
...
export function TicketRegistrationCard({
    ...
    const getStatusColor = (): { bg: string; text: string } => {
        switch (registration.status) {
            ...
            default:
                return { bg: 'var(--ds-color-neutral-surface-default)', text: 'var(--ds-color-neutral-text-subtle)' };
        }
    };
    ...
    return (
        ...
        >
            <div style={containerStyle}>
                {/* Header */}
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 'var(--ds-spacing-2)' }}>
                    <Heading level={3} data-size="xs" style={{ margin: 0, display: 'flex', alignItems: 'center', gap: 'var(--ds-spacing-2)' }}>
                        <TicketIcon />
                        {labels.title}
                    </Heading>
                    ...
                    <ul style={priceListStyle}>
                        {registration.prices.map((price, index) => (
                            <li key={index} style={priceItemStyle}>
                                <div>
                                    <Paragraph data-size="sm" style={{ margin: 0 }}>{price.label}</Paragraph>
                                    {price.description && (
                ...
                {registration.capacity && (
                    <div>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 'var(--ds-spacing-1)' }}>
                            <Paragraph data-size="sm" style={{ margin: 0, display: 'flex', alignItems: 'center', gap: 'var(--ds-spacing-1)' }}>
                                <UsersIcon />
                        ...
                        <div style={capacityBarStyle()}>
                            <div style={capacityFillStyle(registration.capacity.available, registration.capacity.total)} />
                        </div>
                    </div>
                )}
