            ...
            <div style={containerStyle}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 'var(--ds-spacing-2)' }}>
                    ...
                    {schedule.type !== 'single' && (
                        <span style={badgeStyle}>{getScheduleTypeLabel()}</span>
                    )}
                </div>

                {/* Date / Period */}
                <div style={rowStyle}>
                    <span style={iconStyle}><CalendarIcon size={16} /></span>
                    <div>
                        {schedule.type === 'single' ? (
                            ...
                            <Paragraph data-size="sm" style={{ margin: 0 }}>
                                {formatShortDate(schedule.startDate)} – {schedule.endDate ? formatShortDate(schedule.endDate) : '...'}
                            </Paragraph>
                        )}
                    </div>
                </div>
                ...
                {/* Time slots */}
                <div style={rowStyle}>
                    <span style={iconStyle}><ClockIcon size={16} /></span>
                    <div style={timeSlotsStyle}>
                        {schedule.timeSlots.map((slot, index) => (
                            <div key={index} style={{ display: 'flex', alignItems: 'center', gap: 'var(--ds-spacing-2)' }}>
                ...
                {schedule.exceptions && schedule.exceptions.length > 0 && (
                    ...
                    >
                        <Paragraph data-size="xs" style={{ margin: 0, fontWeight: 'var(--ds-font-weight-medium)', color: 'var(--ds-color-warning-text-default)' }}>
